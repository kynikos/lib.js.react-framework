// Generated by CoffeeScript 2.1.1
(function() {
  // This file is part of react-framework
  // Copyright (C) 2018-present Dario Giovannetti <dev@dariogiovannetti.net>
  // Licensed under MIT
  // https://github.com/kynikos/react-framework/blob/master/LICENSE
  var Link, Provider, ReactDOM, Route, Router, applyMiddleware, combineReducers, composeWithDevTools, connect, createBrowserHistory, createFactory, createStore, helpers, push, r, routerMiddleware, routerReducer, thunk, withRouter,
    _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

  require('react');

  ReactDOM = require('react-dom');

  helpers = require('@kynikos/react-helpers');

  ({createFactory, r} = helpers);

  ({Router, Route, withRouter, Link} = require('react-router-dom'));

  ({createStore, combineReducers, applyMiddleware} = require('redux'));

  ({Provider, connect} = require('react-redux'));

  ({routerReducer, routerMiddleware, push} = require('react-router-redux'));

  thunk = require('redux-thunk').default;

  ({composeWithDevTools} = require('redux-devtools-extension'));

  ({createBrowserHistory} = require('history'));

  // Other useful modules:
  // redux-actions
  // reselect
  // jss
  // react-bootstrap
  module.exports = function(reducerMap) {
    var App, history, store;
    history = createBrowserHistory();
    store = createStore(combineReducers(_extends({}, reducerMap, { // noqa
      router: routerReducer
    })), composeWithDevTools(applyMiddleware(thunk, routerMiddleware(history))));
    App = function(root) {
      return r(Provider, {store}, r(Router, {history}, root));
    };
    return {
      ReactDOM,
      App,
      Route: createFactory(Route),
      withRouter,
      connect,
      helpers,
      Link: createFactory(Link),
      history,
      push
    };
  };

}).call(this);
